...
export const forgotPassWord = async (req: Request<{}, {}, IBodyProps>, res: Response) => {
  try {
    ...
    transport.sendMail({
        ...
        html: `<p> Olá, para recuperar sua senha, utilize este token: ${token} </p>`

    }).then (
        () => {
            return res.status(200).json({ ok: { default: 'Email enviado' } });

        }
    )


    console.log('passwordResetToken', req.headers.passwordResetToken);
    console.log('passwordResetExpires', req.headers.passwordResetExpires);
    console.log('idUsuario', req.headers.idUsuario);

    // Retorne o token JWT ou outra resposta adequada
